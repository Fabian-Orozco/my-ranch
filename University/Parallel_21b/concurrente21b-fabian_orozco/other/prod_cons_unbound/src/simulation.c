...
simulation_t* simulation_create() {
  ...
  if (simulation) {
    ...
    sem_init(&simulation->can_consume, /*pshare*/ 0, /*value*/ 0);

    pthread_mutex_init(&simulation->can_access_consumed_count
    , /*pshare*/ NULL);
  ...
  if (threads) {
    for (size_t index = 0; index < count; ++index) {
      if (pthread_create(&threads[index], /*attr*/ NULL, subroutine, data)
         == EXIT_SUCCESS) {
      } else { // no pude crear hilos entonces detiene los hilos
