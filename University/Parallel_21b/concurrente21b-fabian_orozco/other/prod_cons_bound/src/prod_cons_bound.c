  ...
  if (shared_data) {
    ...
    if (error == EXIT_SUCCESS) {
      ...
      if (shared_data->buffer) {
        sem_init(&shared_data->can_produce, /*process_shared*/ 0,
        shared_data->buffer_capacity);

        sem_init(&shared_data->can_consume, /*process_shared*/ 0,0);

        unsigned int seed = 0u;
        // elige un número aleatorio con el procesador de entropía
        // lo guarda en una semilla
        getrandom(&seed, sizeof(seed), GRND_NONBLOCK);
        // usa la semilla como el valor inicial de una sucesión
        srandom(seed);
...
int create_threads(shared_data_t* shared_data) {
  ...
  // SEPARACIÓN DE ASUNTOS / concurrencia de tareas
...
void* produce(void* data) {
  ...
  for (size_t round = 0; round < shared_data->rounds; ++round) {
    for (size_t index = 0; index < shared_data->buffer_capacity; ++index) {
      ...
      // esperar si el semáforo está negativo y también decremento
      ...
      // le avisa al consumidor que puede consumir
    }
  }
